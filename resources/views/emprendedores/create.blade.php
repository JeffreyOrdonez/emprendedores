...
<body class="bg-orange-200 mt-[60px]">
    ...
    <div class="flex justify-center items-center min-h-[80vh]">
        <div class="bg-white bg-opacity-90 rounded-xl shadow-lg p-10 max-w-xl w-full">
            <h1 class="text-3xl font-bold mb-8 text-center">Crear Emprendedor</h1>
            @if(session('error'))
                <div class="bg-red-200 text-red-800 px-4 py-2 rounded mb-4">
                    {{ session('error') }}
                </div>
            @endif
            <form method="POST" action="{{ route('emprendedores.store') }}" class="space-y-6">
                @csrf
                <div>
                ...
                <div>
                  <label class="block mb-2" for="ferias">Ferias a las que desea inscribirse</label>
                 <select id="ferias" name="ferias[]" multiple class="w-full px-4 py-2 rounded border">
                @foreach($ferias as $feria)
              <option value="{{ $feria->id }}" {{ (collect(old('ferias'))->contains($feria->id)) ? 'selected' : '' }}>
